...
// ChatLeft reports that OUR side left a room (kicked, parted, or the server
// closed it): CHAT_LEFT:<id>. Distinct from ChatUpdateBuddy, which is other
// members' presence — a consumer's room state is stale after this event.
...
func parseEvent(raw string) Event {
	...
	case "CHAT_LEFT":
		if f := strings.SplitN(raw, ":", 2); len(f) == 2 {
			return ChatLeft{RoomID: f[1]}
		}
	case "CHAT_IN":
