...
func TestTrimGapIsPerRoomNotGlobalSeq(t *testing.T) {
	cur := time.Date(2026, 8, 14, 12, 0, 0, 0, time.UTC)
	j, err := OpenJournal(filepath.Join(t.TempDir(), "j.db"), func() time.Time { return cur })
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { j.Close() })
	appendMsg := func(room string) {
		t.Helper()
		if _, err := j.Append(room, "x", "chat", "m"); err != nil {
			t.Fatal(err)
		}
	}
	appendMsg("a") // seq 1
	...
	appendMsg("a") // seq 4
	if _, err := j.Trim(24 * time.Hour); err != nil {
		t.Fatal(err) // trims seqs 1-3
	}
	// Room a's reader had read through seq 1; seqs 2-3 belonged to room b
	// (seq is one global AUTOINCREMENT). Nothing of room a's in (1,4] was
	// lost — this used to false-positive as a gap.
	if _, gap, err := j.ReadAfter("a", 1, 10); err != nil || gap {
		t.Fatalf("false gap for room a (err=%v gap=%v)", err, gap)
	}
	// Room b's reader at cursor 2 really did lose seq 3.
	if _, gap, err := j.ReadAfter("b", 2, 10); err != nil || !gap {
		t.Fatalf("missed real gap for room b (err=%v gap=%v)", err, gap)
	}
	// A fresh cursor is a fresh start, never a gap.
	if _, gap, err := j.ReadAfter("a", 0, 10); err != nil || gap {
		t.Fatalf("fresh cursor must not gap (err=%v gap=%v)", err, gap)
	}
}

func TestChatLeftPrunesRoomState(t *testing.T) {
	h := start(t)
	c := newFakeConn()
	h.conns <- c
	waitJoined(t, c, "lobby")
	...
	for {
		...
		if err != nil && strings.Contains(err.Error(), "not joined") {
			break // Say now fails visibly instead of claiming success
		}
		if time.Now().After(deadline) {
			t.Fatalf("say still claims success after ChatLeft: %v", err)
		}
		time.Sleep(5 * time.Millisecond)
	}
	resp, err := h.call(t, Request{Op: "who"})
	...
	if _, ok := resp.Rooms["lobby"]; ok {
		t.Fatal("who still reports the pre-kick roster as live")
	}
}

func TestSecondDaemonRefusesLiveSocket(t *testing.T) {
	h := start(t)
	if _, err := h.call(t, Request{Op: "health"}); err != nil {
		t.Fatal(err)
	}
	d2 := &Daemon{cfg: Config{SocketPath: h.sock}, log: slog.Default()}
	err := d2.serveSocket(context.Background())
	if err == nil || !strings.Contains(err.Error(), "already serving") {
		t.Fatalf("second daemon must refuse a live socket, got %v", err)
	}
	// The first daemon kept its socket.
	if _, err := h.call(t, Request{Op: "health"}); err != nil {
		t.Fatalf("first daemon lost its socket to the refused second: %v", err)
	}
}
