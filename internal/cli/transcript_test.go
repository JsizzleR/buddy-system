...
func turnLine(ts, model, effort string, in, cacheRead, cacheWrite, out int64, sidechain bool, filler int) string {
	return fmt.Sprintf(
		`{"type":"assistant","isSidechain":%t,"timestamp":%q,"gitBranch":"main","effort":%q,"pad":%q,`+
		...
		sidechain, ts, effort, strings.Repeat("p", filler), model, in, cacheWrite, cacheRead, out)
}
...
func TestLastUsageReadsTheNewestRealTurn(t *testing.T) {
	...
	if tailBytes != 64<<10 || maxTailBytes != 512<<10 {
		t.Fatalf("windows are %d/%d, not 64 KB/512 KB: re-size this test's fixtures deliberately",
			tailBytes, maxTailBytes)
	}
	const (
		older = "2026-08-14T11:00:00.000Z"
		newer = "2026-08-14T12:00:00.000Z"
		// Comfortably past a 64 KB window and inside the 512 KB escalation:
		// the largest single line measured in a real transcript here was
		// 267 KB, which is the shape the escalation exists for.
		pastTheWindow = 70 << 10
		// And past the escalation itself.
		pastTheCap = 520 << 10
	)
	for _, tc := range []struct {
	...
	}{
		{
			...
			lines: []string{turnLine(older, "m1", "xhigh", 1, 10, 100, 5, false, 0), turnLine(newer, "m2", "xhigh", 2, 20, 200, 6, false, 0)},
			want:  &usageSample{Model: "m2", Effort: "xhigh", Prompt: 222, CacheRead: 20, CacheWrite: 200, Output: 6},
			why:   "prompt is everything the model was handed: input + cache read + cache write",
		},
		{
			name: "a subagent's turn is not the session's",
			lines: []string{
				turnLine(older, "m1", "xhigh", 1, 10, 100, 5, false, 0),
				turnLine(newer, "sub", "xhigh", 1, 1, 1, 1, true, 0),
			},
			want: &usageSample{Model: "m1", Effort: "xhigh", Prompt: 111, CacheRead: 10, CacheWrite: 100, Output: 5},
			why: "a sidechain record is the subagent's own context; reporting its 3k as the " +
				"session's is how a 90k peer looks free",
		},
		{
			name:  "a record behind one huge tool result is found by the escalation",
			lines: []string{turnLine(older, "m1", "xhigh", 1, 10, 100, 5, false, 0), toolResult(pastTheWindow)},
			want:  &usageSample{Model: "m1", Effort: "xhigh", Prompt: 111, CacheRead: 10, CacheWrite: 100, Output: 5},
			why: "the 64 KB window misses it (issue #9) and the one escalation to 512 KB covers " +
				"the measured shape that caused the miss: a single 267 KB tool result",
		},
		{
			name:  "a record past the escalation is still not reported",
			lines: []string{turnLine(older, "m1", "xhigh", 1, 10, 100, 5, false, 0), toolResult(pastTheCap)},
			want:  nil,
			why: "the escalation is capped, not unbounded: reading back until something turns up " +
				"makes a hook cost a function of how long the session has run",
		},
		{
			name: "the NEWEST record wins, not the last one in the file",
			lines: []string{
				turnLine(newer, "m2", "xhigh", 2, 20, 200, 6, false, 0),
				turnLine(older, "m1", "xhigh", 1, 10, 100, 5, false, 0),
			},
			want: &usageSample{Model: "m2", Effort: "xhigh", Prompt: 222, CacheRead: 20, CacheWrite: 200, Output: 6},
			why: "issue #8: taking the last record POSITIONALLY was an assumption about how the " +
				"harness appends, not a property of what the function returns",
		},
		{
			name:  "an unrecorded effort is absent, not invented",
			lines: []string{turnLine(newer, "m2", "", 2, 20, 200, 6, false, 0)},
			want:  &usageSample{Model: "m2", Prompt: 222, CacheRead: 20, CacheWrite: 200, Output: 6},
			why:   "an older harness records no effort, and a default would be a claim about the session",
		},
		{
		...
		},
		{
			...
			lines: []string{turnLine(older, "m1", "xhigh", 1, 10, 100, 5, false, 0),
		...
		},
		{
			...
			lines: []string{turnLine(older, "m1", "xhigh", 1, 10, 100, 5, false, 0), turnLine("not-a-time", "m2", "xhigh", 9, 9, 9, 9, false, 0)},
			want:  &usageSample{Model: "m1", Effort: "xhigh", Prompt: 111, CacheRead: 10, CacheWrite: 100, Output: 5},
...
func TestLastUsageRefusesImplausibleCounts(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"negative input", turnLine(ts, "m", "xhigh", -1, 0, 0, 0, false, 0)},
		{"negative cache read", turnLine(ts, "m", "xhigh", 1, -10, 0, 0, false, 0)},
		{"a sum that would wrap", turnLine(ts, "m", "xhigh", 1<<62, 1<<62, 0, 0, false, 0)},
	} {
		t.Run(tc.name, func(t *testing.T) {
			path := filepath.Join(t.TempDir(), "t.jsonl")
	...
	if err := os.WriteFile(path, []byte(turnLine(ts, "m", "xhigh", 1, 10, 100, 5, false, 0)+"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestLastUsageTurnTimeIsTheRecordsOwn(t *testing.T) {
	boundedParallel(t)
	path := filepath.Join(t.TempDir(), "t.jsonl")
	if err := os.WriteFile(path, []byte(turnLine("2026-08-14T11:30:00.000Z", "m", "xhigh", 1, 2, 3, 4, false, 0)+"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	got, ok := lastUsage(path)
	if !ok {
