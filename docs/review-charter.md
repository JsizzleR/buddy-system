...
operator's repositories. It has two halves that must not be confused:
...
ledger row entered through the CLI.
...
1. **Enforcement is COOPERATIVE, and saying so is the design.** The PreToolUse gate
   ...
   the worse design.
2. **"No ledger" and "ledger unreadable" are different verdicts and must stay different.**
   Provably not a repo, or a repo that was never `buddy init`ed, means the feature is OFF
...
3. **SQLite, not flat files.** Claims need transactional overlap checks; `O_EXCL` files
   lose to ABA, partial writes, and multi-scope atomicity. The driver is
...
4. **The ledger lives at `<git-common-dir>/buddy.db`.** The common dir, not `.git`, so
   that `git worktree` checkouts of one repo share one ledger. It is machine-local and
   never committed.
5. **Scopes are exact paths, repo-relative, slash-separated.** Containment is
...
6. **One folding rule: `strings.ToLower(norm.NFC.String(s))`.** macOS aliases a repo root
   several ways and its default volume is case-insensitive. Never `strings.EqualFold`,
   never a second normalization. Accepted residual, already documented: this over-merges
   on a case-SENSITIVE volume mounted under a repo.
...
8. **Room digests are never auto-injected into an agent's context.** Only operator inbox
   messages are. Visibility means the operator sees the room, not that every agent ingests
   it. Context cost is a first-class constraint here, not an afterthought.
9. **Loopback by default, everywhere.** The chat servers run unauthenticated only under
   ...
   boundary.
10. **Sessions are identified by `(session_id, incarnation)`.** A delayed `bye` from a dead
    incarnation must not orphan a live one; a delayed `beat` must not resurrect an ended
    session. PID is diagnostic only, never authoritative.
...
12. **Stale claims are never auto-reaped.** Sweeping an open claim because its owner went
    quiet is how a live session loses its reservation. `sweep --force` is the operator's
    explicit act.
...
- IRC is the daily driver (`ergo`, loopback :6667, IRCv3 `echo-message`). The TOC/AIM
  backend stays behind `--backend toc`. UTF-8 is native on IRC; the CP1252 conversion is a
  TOC-only concern.
...
- Peers address each other by **claim slug**, not by session id or label. A `mentions_me`
  filter built from id and label scored 0 matches on 2313 live messages.
