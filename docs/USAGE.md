...
The [README](../README.md) gets you installed and wired. This page is the
reference behind it: what each feature does, the failure that motivated it, and
what it deliberately does **not** do. `buddy --help` and `buddy <verb> --help`
are always current for exact syntax.

A reference like **(D-037)** names an entry in the [decision record](decisions.md),
which holds the measurement and the rejected alternatives behind each rule. A
**#NN** is a [GitHub issue](https://github.com/JsizzleR/buddy-system/issues).
The [field notes](wishlist.md) are the problem reports many of these features
answer.

## Contents

- [Hooks in detail](#hooks-in-detail)
- [Upgrading and the ledger's schema](#upgrading-and-the-ledgers-schema)
- [Claims in depth](#claims-in-depth)
- [The operator's brake — `pause`, `resume`, `sweep`](#the-operators-brake--pause-resume-sweep)
- [Whose is this uncommitted change? — `buddy whose`](#whose-is-this-uncommitted-change--buddy-whose)
- [Messages — `buddy msg`](#messages--buddy-msg)
- [One session, every register — `status` and `who`](#one-session-every-register--status-and-who)
- [The roster — `buddy sessions`](#the-roster--buddy-sessions)
- [Authority files — `buddy authority`](#authority-files--buddy-authority)
...
The wiring itself is in the [README](../README.md#3-hook-wiring). What each
line is for, and why it behaves as it does.

**Installation is the opt-in.** The `[ -x ]` guard means a machine without the
binary simply doesn't have the feature; but once a repo is `buddy init`-ed, the
gate fails **closed**: an unreadable or corrupt ledger denies mutating tools
rather than shrugging. And enforcement is **cooperative**. The gate adjudicates
the paths tools declare (including cross-repo targets, against the *target*
repo's ledger), but it cannot bind processes that bypass the harness. It is a
seatbelt for agents, not a sandbox against them.
...
SIGKILLs the result — measured, exit 137 on the very next run.
`go build -o ~/bin/buddy ./cmd/buddy` replaces the file, and is what
`install.sh` does; `cp new ~/bin/buddylist` is not safe. It matters because the hook lines end in `exit 0`: a killed binary is
...
every other lane, which is how two rules in the [field notes](wishlist.md) (§7) lost their home.
...
## The operator's brake — `pause`, `resume`, `sweep`
...
buddy sweep --force               # also frees claims whose holder went silent: the operator's act
```

A paused session's next mutating tool call is denied, with the note. `sweep
--dry-run` is the real sweep rolled back inside its transaction, so what it
prints is what a sweep would do.

## Whose is this uncommitted change? — `buddy whose`
...
every recipient rationally ignores it — which is what happened in one run, to
...
after it. It is signed with the label because a message once went out signed
with a claim slug whose claim had been *refused* — so it never opened, so the
reply bounced with `no such target`, at the one party trying to unblock the
sender.
...
because a message shown cut is the same silent failure as one never sent.

**Targets.** `pause`, `resume` and `msg` all take the same target: `all`, a
session id, a label, an `s-<8hex>` short form, or an **open claim slug** — slugs
included because that is how peers address each other. Matching is exact.
Anything else exits non-zero and writes nothing, rather than queueing a row that
would match nothing and reporting success. `buddy msg all` goes to the sessions
live at send time; a session created later never inherits it, and an
undelivered broadcast expires after 24 hours. `hello` warns, once, when your
...
**What a send reports.** `msg` reports what the ledger holds about its
...
that had gone away all reported that (#24, D-032). Now one observation
...
## One session, every register — `status` and `who`
...
# * repo/s-16c16a94  (16c16a94-…)  live  started 3h  seen 2m  idle 2m  pid 30479  pane tmux:%3
# CLAIMS HELD  2
...
- `idle 7m` — the session finished a turn that long ago and has run no tool
  ...
  **Absence is not evidence of busy.** `Stop` is a hook line a machine may not
  ...
  needs a poke, which `msg` names the address for
  ([Waking a session at its prompt](#messages--buddy-msg), D-039). A session that
...
- `base 079dd6a7 (2 ahead, 3 behind main, 12m ago)` — the commit the
  ...
  and `2 ahead of main` are the other shapes. Measured case (#28): a
...
- `…; carries 1 commit main DROPPED: 8ace1954` — the base is built on a commit
  ...
  ([field notes](wishlist.md) §16): a peer rebased silently onto a commit main had amended
...
## Authority files — `buddy authority`
...
## Waiting without going cold — `buddy wait`
...
## Resource slots — a claim on `.buddy/slot/<name>`
...
authorized that load, and only the coordinator could see it (#27).
...
## One long run closes out several sessions

`buddy wait --ready` and `buddy release --outcome` (D-049). A slot takes
turns. When the guarded job is a 65-minute hermetic tier, taking
...
it — "I'm done in 10, hold the run for me" — and before D-049 that protocol lived
...
Installing it in your repo is in the [README](../README.md#4-the-commit-gate-optional).
In the buddy-system checkout itself, `sh scripts/setup-clone.sh` sets
`core.hooksPath` to `.githooks` and inits the ledger.
...
so there is a check there too. Once it is installed, `git commit` reports any staged path sitting inside another
...
The full `buddylist` command surface (the [README](../README.md#presence-the-fun-half)
covers running a server and registering the MCP tools):
...
buddylist dm alice "nightly is RED"           # exits non-zero if nobody is there to receive it
```
...
### Journal, cursors and alerts

- Inbox delivery (`buddy msg`) is at-least-once.
